    ...
    @foreach($preiskalkulator->kategorien as $kategorie)
        ...
        <div class="list-group">
            ...
            <a href="#" onclick="openUrl('{{ $url."&kategorie_".$kategorie->id."=".$auswahl->id }}')" class="list-group-item list-group-item-action @if($selected_id == $auswahl->id) active @endif">
                {{ $auswahl->name }}
                <span onclick="event.stopPropagation();editAuswahl({{ $auswahl->id }},'{{ $auswahl->name }}','{{ $auswahl->name_en }}','{{ $auswahl->preis }}','{{ $auswahl->order }}','{{ $auswahl->description }}','{{ $auswahl->image }}')" class="btn btn-primary"><i class="fas fa-pencil-alt"></i></span>
...
<script>
    ...
    function editAuswahl(auswahl_id, name, name_en, preis, order, description, image) {
        $('#modalAuswahl').modal('show');
