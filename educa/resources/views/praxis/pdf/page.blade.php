...
<main>
    ...
    <h2>Praxis-Einsatzplan<small> Stand: {{ date("d.m.Y") }}</small></h2>
    <h4>Einsatzplan für: {{ $schuler->displayName }}</h4>
    ...
    <table class="table table-striped" style="width: 100%; font-size: 12px;">
        ...
        @foreach($abschnitte as $selectAbschnitt)
         <!--   <tr>
                <td colspan="7">
                   Abschnitt: <b>{{ $selectAbschnitt->name }}</b>, {{ $selectAbschnitt->inhalt }}
                </td>
            </tr> -->
        ...
        <tr>
            ...
            @endif
            @if($einsatz->kontakt != null)
            <td>{{ $einsatz->kontakt->name }}</td>
            @else
                <td><i>Kein Ansprechpartner</i></td>
            @endif
