...
use StuPla\CloudSDK\calendarful\Recurrence\RecurrenceFactoryInterface;
...
class Calendar implements CalendarInterface
{
	...
	public function populate(EventRegistryInterface $eventsRegistry, \DateTime $fromDate, \DateTime $toDate, $limit = null, array $extraFilters = array())
	{
		...
		if ($this->recurrentEvents = $eventsRegistry->getRecurrentEvents($filters)) {
		    $filters['parent_ids'] =  array_map(function($o) { return $o->getId();}, $this->recurrentEvents);
		...
		$this->removeVactationEvents($eventsRegistry->getVactionTimes($filters));
    ...
    protected function removeVactationEvents($vactationTimes)
    {
        // Remove events that do not occur within the date range
        $this->allEvents = array_filter($this->allEvents, function ($event) use ($vactationTimes) {

            foreach ($vactationTimes as $vactationTime)
            {
                $startVactationTime = Carbon::parse($vactationTime->start);
                $endVactationTime = Carbon::parse($vactationTime->end);

                // Liegt in den Ferien und startet da auch nicht
                if (($event->getStartDate() >= $startVactationTime && $event->getEndDate() <= $endVactationTime)
                && ($event instanceof RecurrentEventInterface && $event->getRecurrenceStart() != null && ($event->getRecurrenceStart() <= $startVactationTime || $event->getRecurrenceStart() >= $endVactationTime))) {
                    return false;
                }
            }
            return true;
