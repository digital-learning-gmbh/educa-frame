...
use StuPla\CloudSDK\calendarful\Recurrence\RecurrenceInterface;
...
class Weekly implements RecurrenceInterface
{
	...
	public function generateOccurrences(Array $events, \DateTime $fromDate, \DateTime $toDate, $limit = null)
	{
		...
		foreach ($weeklyEvents as $weeklyEvent) {
			...
			$startMarker = clone($weeklyEvent->getStartDate());
			...
			$dateInterval = new \DateInterval('P'.$weeklyEvent->getRecurrenceTurnus().'W');
			...
			foreach ($datePeriod as $date) {
				if (($limit and ($limit === $limitMarker)) or ($date > $endMarker)) {
					break;
				}
                ...
                $newWeeklyEvent->setRecurrenceStart($startMarker);
