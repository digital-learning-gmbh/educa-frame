...
use StuPla\CloudSDK\rocketchat\Exceptions\GroupActionException;
...
class GroupClient extends Client
{
    ...
    const API_PATH_GROUP_MEMBERS       = 'groups.members';
    ...
    public function delete(Room $group)
    {
        if (!$group->getId()) {
            throw new GroupActionException('group ID not specified');
        }

        $response = $this->request()->post($this->apiUrl(self::API_PATH_GROUP_DELETE))
            ->body(['roomId' => $group->getId()])
            ->send();
    ...
    {
        $response = $this->request()->get($this->apiUrl(self::API_PATH_GROUP_MEMBERS,['roomId' => $roomId]))
            ->send();

        return $this->handleResponse($response, new GroupActionException(), "members");
    }
}
