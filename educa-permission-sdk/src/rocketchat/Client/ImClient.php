...
{
    const API_PATH_IM_CREATE          = 'im.create';
    ...
    public function create($usernames)
    {
        if (!$usernames) {
            throw new ImActionException("Username(s) not specified.");
        }

        $response = $this->request()->post($this->apiUrl(self::API_PATH_IM_CREATE))
            ->body(['usernames' => $usernames])
            ->send();

        $groupData = $this->handleResponse($response, new ImActionException(),['room']);
    ...
    {
        $response = $this->request()->get($this->apiUrl(self::API_PATH_IM_LIST,["sort" => '{ "_updatedAt" : -1  }']))->send();
